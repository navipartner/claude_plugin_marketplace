...
**Path:** `${CLAUDE_PLUGIN_ROOT}/bin/bcdev`
...
${CLAUDE_PLUGIN_ROOT}/bin/bcdev symbols -appJsonPath "/path/to/app.json"

# With country-specific packages (e.g., us, de, dk)
${CLAUDE_PLUGIN_ROOT}/bin/bcdev symbols -appJsonPath "/path/to/app.json" -country us
```

**Server mode (opt-in):**
```bash
${CLAUDE_PLUGIN_ROOT}/bin/bcdev symbols \
...
**Usage:**
```bash
${CLAUDE_PLUGIN_ROOT}/bin/bcdev compile \
  -appJsonPath "/path/to/app.json"
```
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/bcdev publish \
  -launchJsonPath "/path/to/.vscode/launch.json" \
  -launchJsonName "Your Configuration Name" \
...
**Usage (run all tests):**
```bash
${CLAUDE_PLUGIN_ROOT}/bin/bcdev test \
  -launchJsonPath "/path/to/.vscode/launch.json" \
  -launchJsonName "Your Configuration Name" \
  -all \
  -Username "bcuser" \
